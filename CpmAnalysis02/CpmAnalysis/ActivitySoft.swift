...
class Project {
    ...
    private func printProject(activities: [ActivitySoft], criticalActivities: [ActivitySoft]) {
        ...
        for activity in activities {
            ...
            if (activity.actualFinish != 0) {
                result.append("\(activity.actualStart)     \(activity.actualFinish)\n")
                result.append("--------")
                result.append("\n")
                result.append(" \(activity.description):\(activity.id)\n " )
                result.append("  D:\(activity.duration)\n " )
                result.append("--------")
                result.append("\n")
            
            } else if (activity.actualStart != 0) {

                result.append("\(activity.actualStart)     \(activity.earlyFinish)\n")
                result.append("--------")
                result.append("\n")
                result.append(" \(activity.description):\(activity.id)\n " )
                result.append("  D:\(activity.duration)\n " )
                result.append("--------")
                result.append("\n")
                result.append("\(activity.actualStart) \(activity.totalFloat)/\(activity.freeFloat) \(activity.lateFinish)\n")
                
                
            } else {
                result.append("\(activity.earlyStart)     \(activity.earlyFinish)\n")
                result.append("--------")
                result.append("\n")
                result.append(" \(activity.description):\(activity.id)\n " )
                result.append("  D:\(activity.duration)\n " )
                result.append("--------")
                result.append("\n")
                result.append("\(activity.lateStart) \(activity.totalFloat)/\(activity.freeFloat) \(activity.lateFinish)\n")
            }
    ...
    private func printCPonly(activities: [ActivitySoft], criticalActivities: [ActivitySoft]) {
            result.removeAll()
            criticalPathFind(activities: activities, criticalActivities: criticalActivities)
            result.append("Critical Paths \n")
            for criticalPath in criticalPaths {
    ...
    public func criticalprintonly (startDate: Int32) {
            for schedule in schedules {
                let finishDay = forwardPass(activities: schedule.schedule , startDay: startDate)
                backwardPass(activities: schedule.schedule, finishDay: finishDay)
                let criticalActivities = floatCalculation(activities: schedule.schedule)
                printCPonly(activities: schedule.schedule, criticalActivities: criticalActivities)
            }
        }
}
